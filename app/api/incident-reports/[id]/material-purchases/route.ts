...
export async function GET(
  ...
  try {
    ...
    const purchases = await getMaterialPurchases("incident_report", id);
    return Response.json({ purchases, s3Configured: isS3Configured() });
  } catch (error) {
    console.error("Error fetching incident material purchases:", error);
...
export async function POST(
  ...
  try {
    ...
    console.error("Error creating incident material purchase:", error);
...
export async function DELETE(
  ...
  try {
    ...
    if (
      !deletedPurchase ||
      deletedPurchase.entity_type !== "incident_report" ||
      deletedPurchase.entity_id !== id
    ) {
    ...
    console.error("Error deleting incident material purchase:", error);
